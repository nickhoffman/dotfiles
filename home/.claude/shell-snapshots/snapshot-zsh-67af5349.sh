...
gcmput () {
	(
		set -x
		git --no-pager commit -m "put this in $@"
	)
}
gcobt () {
	git --no-pager checkout -b $1 -t origin/$1
}
gdnolock () {
...
git_files_changed_in_branch () {
	current_branch="$(git rev-parse --abbrev-ref HEAD)" 
	common_ancestor="$(git merge-base ${current_branch} master)" 
	git --no-pager diff --name-only $current_branch $common_ancestor
}
git_main_branch () {
	command git rev-parse --git-dir &> /dev/null || return
	local ref
	...
	case $COMMAND in
		...
		("alias") local NVM_ALIAS_DIR
			...
			then
				nvm unalias "${ALIAS}"
				return $?
			elif echo "${ALIAS}" | egrep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox} --color -q "#"
			then
...
rt () {
	if egrep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox} --color -q "spring-commands-testunit" Gemfile
	then
		bundle exec spring testunit $@
	elif egrep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox} --color -q "spring-commands-rspec" Gemfile
	then
...
alias -- gfom='git fetch --prune origin master '
...
alias -- gl1='git log --pretty=tformat:'\''%C(yellow)%h%Creset%x09%an%x09%ad%x09%s'\'' --date=short --reverse '
...
alias -- gsns='git show --name-status --reverse  '
...
if ! command -v rg >/dev/null 2>&1; then
  alias rg='/Users/nick/.nvm/versions/node/v23.5.0/lib/node_modules/@anthropic-ai/claude-code/vendor/ripgrep/arm64-darwin/rg'
fi
export PATH='/Users/nick/.nvm/versions/node/v23.5.0/bin:/opt/homebrew/bin:/opt/homebrew/sbin:/Users/nick/.asdf/shims:/usr/local/bin:/System/Cryptexes/App/usr/bin:/usr/bin:/bin:/usr/sbin:/sbin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/local/bin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/bin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/appleinternal/bin:/Library/Apple/usr/bin:/Users/nick/.nvm/versions/node/v23.5.0/bin:/opt/homebrew/opt/asdf/libexec/bin'
